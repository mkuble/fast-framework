package com.fast.framework.data.cache;
...
import com.fast.framework.data.exception.FastRedisException;
...
/**
 * Redis Client帮助类
 * @version 1.0.0 2017年4月26日 下午4:56:32
 */
public class RedisClient {
	...
	// 最大空闲连接数
	private static final int MAX_IDLE_COUNT = 8;
	...
	public static void init(String ip, int port, String password, int maxIdleCount, int maxTotalCount, int maxWaitMillis) {
		...
		// 创建连接池
		if (FastStringUtils.isEmpty(password)) {
	...
	public static boolean existsKey(String key) {
		...
		try {
			...
			e.printStackTrace();
			throw new FastRedisException(e.getMessage(), e);
		} finally {
			closeResource(jedis);
		}
	}
	...
	public static <T> void setValue(String key, T value, int expireSeconds) {
		if (value != null) {
			setValue(key, JSON.toJSONString(value), expireSeconds);
		}
	}
	...
	public static <T> void setValue(String key, T value, int expireSeconds, boolean needSetFlag) {
		if (value != null) {
			setValue(key, JSON.toJSONString(value), expireSeconds, needSetFlag);
		}
	}
	...
	public static <T> void setValue(String key, T value, String expireTime) {
		if (value != null) {
			setValue(key, JSON.toJSONString(value), expireTime);
		}
	}
	...
	public static <T> boolean setnx(String key, T value, int expireSeconds) {
		return setnx(key, JSON.toJSONString(value), expireSeconds);
	}
	...
	public static <T> T getListFirstElement(String key, Class<T> c) {
		Jedis jedis = pool.getResource();
		try {
			return JSON.parseObject(jedis.lindex(key, 0), c);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		} finally {
	...
	public static <T> T getListPop(String key, Class<T> c) {
		Jedis jedis = pool.getResource();
		try {
			return JSON.parseObject(jedis.lpop(key), c);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		} finally {
	...
	public static <T> T getListLastElement(String key, Class<T> c) {
		Jedis jedis = pool.getResource();
		try {
			return JSON.parseObject(jedis.lindex(key, -1), c);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		} finally {
	...
	public static <T> void removeValueFromList(String key, T... values) {
		...
		try {
			for (T v : values) {
				jedis.lrem(key, 0, JSON.toJSONString(v));
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw new FastRedisException(e.getMessage(), e);
	...
	public static <T> void addToHashMap(String hashKey, String key, T value) {
		Jedis jedis = pool.getResource();
		try {
			jedis.hset(hashKey, key, JSON.toJSONString(value));
		} catch (Exception e) {
			e.printStackTrace();
			throw new FastRedisException(e.getMessage(), e);
		} finally {
	...
	public static <T> T getValueFromHashMap(String hashKey, String key, Type typeOfT) {
		...
		try {
			...
			return JSON.parseObject(s, typeOfT);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		} finally {
	...
	public static <T> Map<String, T> getAllFromHashMap(String hashKey, Class<T> c) {
		...
		try {
			...
			for (String key : map.keySet()) {
				T value = JSON.parseObject(map.get(key), c);
				tMap.put(key, value);
			}
			return tMap;
		} catch (Exception e) {
	...
	public static void removeHashMap(String hashKey) {
		...
		if (FastStringUtils.isEmpty(hashKey)) {
			return;
		}
